...
# --------------------------
def train_agent(env, agent, num_episodes=500, gamma=0.99, print_interval=10, record_history = 1, l2_actor=1e-4, l2_critic=1e-4, batch_size=10):
    """
    ...
    Args:
        ...
        batch_size: Number of episodes to process in each training batch.
    ...
    if num_episodes % batch_size != 0:
        print(f"Warning: num_episodes ({num_episodes}) no es divisible por el tamaño del lote ({batch_size}). Se ajustará al múltiplo más cercano.")
    ...
    for batch_idx in range(1, num_batches + 1):
        ...
        # Inicializar acumuladores a None. Se crearán de forma perezosa.
        batch_actor_grads = None
        batch_critic_grads = None
        batch_actor_loss = 0.0
        batch_critic_loss = 0.0

        # --- BUCLE INTERNO: Recolectar datos de `batch_size` episodios ---
        for _ in range(batch_size):
            
            # ... (la simulación del episodio y la preparación de tensores no cambian) ...
            state, info = env.reset()
            done = False
            states, actions, rewards = [], [], []
            actor_hidden_states_ep, critic_hidden_states_ep = [], []
            current_actor_hidden, current_critic_hidden = None, None
            
            while not done:
                action, _, actor_hidden_states_after_select = agent.select_action(state, actor_hidden_states=current_actor_hidden, training=True)
                ...
                if actor_is_recurrent: r_t = actor_hidden_states_ep[-1]
                else:
                    state_tensor = tf.expand_dims(tf.expand_dims(tf.convert_to_tensor(state, dtype=tf.float32), 0), 1)
                    r_t = agent.actor.get_hidden_dense(state_tensor)[0, 0, :].numpy()
                _, critic_hidden_state_after_eval = agent.evaluate_critic_step(r_t, np.eye(act_size, dtype=np.float32)[actions[-1]], critic_hidden_states=current_critic_hidden, training=False)
                if critic_is_recurrent:
                    critic_hidden_states_ep.append(critic_hidden_state_after_eval[0].numpy().flatten())
                state = next_state

            returns = discount_rewards(rewards, gamma)
            states_sequence_tensor = tf.expand_dims(tf.convert_to_tensor(states, dtype=tf.float32), 0)
            actions_tensor = tf.expand_dims(tf.convert_to_tensor(actions, dtype=tf.int32), 0)
            returns_tensor = tf.expand_dims(tf.convert_to_tensor(returns, dtype=tf.float32), 0)
            dense_feats = None
            if not actor_is_recurrent:
                dense_hidden_seq = agent.actor.get_hidden_dense(states_sequence_tensor)
                dense_feats = dense_hidden_seq[0].numpy()
            critic_inputs_list = []
            for t in range(len(actions)):
                feat = actor_hidden_states_ep[t] if actor_is_recurrent else dense_feats[t]
                curr_a = np.eye(act_size, dtype=np.float32)[actions[t]]
                critic_inputs_list.append(np.concatenate([feat, curr_a]))
            critic_inputs = tf.convert_to_tensor([critic_inputs_list], dtype=tf.float32)

            # --- CÁLCULO DE GRADIENTES ---
            with tf.GradientTape() as tape_actor:
                all_probs, _ = agent.actor(states_sequence_tensor, training=True)
                ...
                all_values, _ = agent.critic(critic_inputs, training=False)
                advantage = returns_tensor - tf.stop_gradient(tf.squeeze(all_values, axis=-1))
            ...
            with tf.GradientTape() as tape_critic:
                all_values_train, _ = agent.critic(critic_inputs, training=True)
                values = tf.squeeze(all_values_train, axis=-1)
            ...
            # --- INICIALIZACIÓN PEREZOSA Y ACUMULACIÓN DE GRADIENTES ---
            
            # Si es el primer episodio, ahora que el modelo está construido, crea los acumuladores.
            if batch_actor_grads is None:
                batch_actor_grads = [tf.zeros_like(v) for v in agent.actor.trainable_variables]
                batch_critic_grads = [tf.zeros_like(v) for v in agent.critic.trainable_variables]
            
            # Acumular gradientes. Se comprueba si el gradiente es None.
            for i, grad in enumerate(actor_grads):
                if grad is not None:
                    batch_actor_grads[i] += grad
            
            for i, grad in enumerate(critic_grads):
                if grad is not None:
                    batch_critic_grads[i] += grad

            batch_actor_loss += actor_loss.numpy()
            batch_critic_loss += critic_loss.numpy()
            
            # --- Guardar historial para cada episodio ---
            total_rewards_history.append(sum(rewards))

        # --- FIN DEL LOTE: PROMEDIAR Y APLICAR GRADIENTES ---
        
        avg_actor_grads = [grad / batch_size for grad in batch_actor_grads]
        avg_critic_grads = [grad / batch_size for grad in batch_critic_grads]
        avg_actor_loss = batch_actor_loss / batch_size
        avg_critic_loss = batch_critic_loss / batch_size
        
        avg_actor_grads, _ = tf.clip_by_global_norm(avg_actor_grads, clip_norm=1.0)
        avg_critic_grads, _ = tf.clip_by_global_norm(avg_critic_grads, clip_norm=1.0)
        agent.actor_optimizer.apply_gradients(zip(avg_actor_grads, agent.actor.trainable_variables))
        agent.critic_optimizer.apply_gradients(zip(avg_critic_grads, agent.critic.trainable_variables))
        ...
        # Imprimir progreso cada `print_interval` lotes
        if batch_idx % print_interval == 0:
            # Calcular recompensa media del último lote
            avg_reward = np.mean(total_rewards_history[-batch_size:])
            print(f"Batch {batch_idx}/{num_batches}\tAvg Reward: {avg_reward:.2f}\t"
